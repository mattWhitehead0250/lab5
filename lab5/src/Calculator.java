...
public class Calculator
{
	...
	{
		int value = Integer.parseInt(tokens[1]); // Throws NumberFormatException if the second token is not an int value.
		// TODO: complete this...
		//check to make sure all input is valid input
		if (!tokens[0].equalsIgnoreCase("negate")&&!tokens[0].equalsIgnoreCase("halve")) {
			throw new CalculatorException("Improper Command");
		}
		//performs calculation if first token is negate
		if (tokens[0].equalsIgnoreCase("negate")) {
			return -value;
		}
		//performs calculation if first token is halve
		if (tokens[0].equalsIgnoreCase("halve")) {
			return value / 2;
		}
		//had to add to get it to compile
		return 0;
	}
			...
			throws ArithmeticException, NumberFormatException, CalculatorException
	{
		...
		int result = 0;
		//same format check as above and calculations
